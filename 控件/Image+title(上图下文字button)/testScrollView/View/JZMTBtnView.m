...
-(id)initWithFrame:(CGRect)frame title:(NSString *)title imageStr:(NSString *)imageStr{
    ...
    if (self) {
        ...
        CGFloat imageWidth = frame.size.height / 10 * 6;
        CGFloat imageHight = frame.size.height / 10 * 6;
        CGFloat imageToLabelSpace = frame.size.height / 100 * 8;
        ...
        UILabel *titleLable = [[UILabel alloc] initWithFrame:CGRectMake((buttonWidth - labelWidth)/2, toTopHeight + imageHight, labelWidth, 20)];
        titleLable.text = title;
        titleLable.textAlignment = NSTextAlignmentCenter;
        titleLable.font = [UIFont systemFontOfSize:13];
        [self addSubview:titleLable];
